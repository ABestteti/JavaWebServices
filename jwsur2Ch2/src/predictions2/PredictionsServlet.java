...
public class PredictionsServlet extends HttpServlet {
	...
	// POST /predictions2
	// HTTP body should contain two keys, one for the predictor ("who")
	// and another for the prediction ("what").
	@Override
	public void doPost(HttpServletRequest request, HttpServletResponse response) {
		String who  = request.getParameter("who");
		String what = request.getParameter("what");
		
		// Are the data to create a new predict present?
	...
	public void doPut(HttpServletRequest request, HttpServletResponse response) {
		// A workaround is necessary for a PUT because neither Tomcat nor Jetty
		...
		try {
			...
			// parameters: the ID and either who or what. Assume, further, that the
			// id comes first. From the client side, a hash character # separates the
			// id and the who/what, e.g., id=33#who=Homer Allison
		...
		if (p == null) {
			// Not found?
			String msg = key + " does not map to a Predicion!\n";
			sendResponse(response, predictions.toXML(msg), false);
		}
		else {
			// Found
	...
	public void doDelete(HttpServletRequest request, HttpServletResponse response) {
		...
		// Only on prediction can be deleted at a time.
		if (key == null) {
			throw new HTTPException(HttpServletResponse.SC_BAD_REQUEST);			
		}
